...
| `scripts/backup-envelope.mjs` (×2, the drill both backup tools share) | `127.0.0.1` — the throwaway server the restore drill just started | Proving a backup boots, not just decrypts (`docs/BACKUPS.md`) | Nothing: a `GET /healthz` and a page load | Operator tool; never runs in the service |
| `scripts/backup-postgres.mjs` | The database the operator's URL file names, through `pg_dump`, `pg_restore` and the `pg` driver | Taking, verifying, restoring and drilling a PostgreSQL backup (ADR-0115) | Credentials as `PG*` variables to a child process; never on a command line | Operator tool; never runs in the service |
