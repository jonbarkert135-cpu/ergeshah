...
| MARKETPLACE / SELLERS / ORDERS / REVIEWS | `routes/market.ts`, `routes/bonds.ts`, `routes/evidence.ts`, `lib/reputation.ts`, `lib/bonds.ts`, `lib/search.ts`, `lib/evidence.ts` | listings, applications, the order state machine, reviews and reputation, the digests a disputing party commits to without uploading the file (ADR-0074), and the bond a seller stakes against their own conduct (ADR-0086) |
