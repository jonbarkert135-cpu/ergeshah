...
| JPEG segment structure (JFIF, `APPn`, `COM`, entropy-coded scan) | ITU-T T.81 / ISO 10918-1, and the JFIF specification | Metadata stripping in `src/shared/images.ts` | `test/images.test.ts` walks a container built segment by segment and asserts the scan survives byte for byte |
| PNG chunk layout and the ancillary chunks that carry text and EXIF | W3C PNG (Third Edition), ISO/IEC 15948 | The same stripper | The same suite, with real CRC32 values from `node:zlib` so the fixture is a valid file |
| WebP RIFF container, `VP8X` flags, `EXIF` and `XMP ` chunks | Google's WebP container specification | The same stripper | The same suite: the chunks are gone, the flags that announced them are cleared, and the RIFF size matches what is there |
