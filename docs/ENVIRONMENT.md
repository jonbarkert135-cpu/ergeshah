...
| `CANARY_FINGERPRINT` | *unset* | The one OpenPGP fingerprint whose signature this deployment will publish as its canary (OPS-7, ADR-0099). 40 or 64 hex characters; spaces and colons from a `gpg --fingerprint` paste are ignored. Unset means this deployment publishes no canary at all: `POST /api/admin/canary` refuses and the client shows nothing, which is the honest state — a canary nobody signs looks like one and is not |
