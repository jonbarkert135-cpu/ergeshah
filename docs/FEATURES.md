...
| Key directory and prekeys | `messaging.ts` | `routes/keys.ts` (7 routes) | `devices`, `one_time_prekeys`, `vaults` | Session required to publish; bundles and identity keys readable by any account, rate-limited | `protocol.test.ts`, `verification.test.ts`, `trust.test.ts`, `client.test.ts` | `docs/CRYPTO.md`, `docs/API.md` |
...
| Sealed sender (single-use send tokens) | `client/api.ts`, `client/messaging.ts` (the token pouch) | `routes/messages.ts`, `lib/send_tokens.ts`, `scripts/incident.mjs send-tokens:revoke` | `send_tokens` (hash, expiry), `send_token_epoch` (global revocation floor) | Token minting needs a session; sending needs an unspent token under the current epoch, and carries no cookie; only an operator raises the floor | `sealed_sender.test.ts` | ADR-0084, ADR-0111 |
