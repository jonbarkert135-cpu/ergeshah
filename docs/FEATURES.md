...
| --- | --- | --- | --- | --- | --- | --- |
| Registration, login, sessions | `views/auth.ts`, `state.ts` | `routes/auth.ts` (10 routes) | `users`, `sessions`, `devices` | Public by allowlist; everything else needs a session cookie + CSRF | `auth.test.ts`, `sessions.test.ts`, `authorization.test.ts` | `docs/API.md`, `docs/PRIVACY.md` |
...
| PGP second factor, and key rotation | `views/security.ts` | `routes/recovery.ts` | `users.pgp_public_key`, `auth_challenges` | Password *and* signature; replacing or removing a key needs a signature from the key being replaced or removed; server-side OpenPGP only | `pgp.test.ts` | `docs/CRYPTO.md`, ADR-0087, ADR-0088 |
...
| Attachments | `views/chat.ts` | `routes/deliveries.ts` | `attachments` | Owner or addressee; size, rate, per-account byte budget and free-space limits | `attachments.test.ts`, `uploads.test.ts`, `limits.test.ts` | `docs/API.md`, `docs/STORAGE.md`, `docs/THREAT_MODEL.md` §Hostile uploads |
