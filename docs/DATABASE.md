...
| `send_token_epoch` | single row: `min_epoch` | The sealed-sender revocation floor (MD-5, ADR-0111). A token carries the epoch it was minted under; a spend is refused when that epoch is below `min_epoch`. Raising it (`scripts/incident.mjs send-tokens:revoke`) invalidates every outstanding token in one write. Global and coarse on purpose — it holds no account, and is not per-batch, so it is not a grouping key |
