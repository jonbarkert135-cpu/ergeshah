...
| `fuzz.test.ts` | **Security** | Points 160 and 161: seeded corpora — empty, huge, truncated, duplicated, nested, invalidly encoded, boundary — against the image walker, the padding scheme, base64url, every validator, money parsing, recovery phrases, file names, ratchet frames and the cookie header, plus the whole route table with hostile bodies and query strings. The property is "reject safely": an `Error`, in bounded time, and never a 500 |
| `authz_fuzz.test.ts` | **Security** | Points 162–164: the credential matrix generated from the route table (expired session, revoked session, suspended account, wrong role, moderator on an admin route, worker token), every shape of malformed identifier, every real identifier in the deployment presented by a stranger, and a mass-assignment sweep that reads the database before and after |
| `security_pipeline.test.ts` | **Security** | Points 149–153: every source rule fires on a planted example and stays quiet on correct code, the findings register parses and blocks an open CRITICAL or HIGH, every fixed finding names a test that exists and appears in the changelog, suppressions carry an owner and an unexpired review date, and no stage of the pipeline needs a paid service |
