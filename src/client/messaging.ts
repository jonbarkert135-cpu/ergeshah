...
import { decodeIdentity, encodeIdentity, persistVault, state, type Conversation } from "./state.ts";
...
export async function startConversation(peer: string): Promise<Conversation> {
  const existing = conversationWith(peer);
  if (existing) return existing;
  const conversation: Conversation = {
    channel: toBase64Url(randomBytes(24)),
    peer,
    messages: [],
    sessions: {},
  };
...
export async function sendMessage(conversation: Conversation, text: string): Promise<void> {
  ...
  const plaintext = JSON.stringify({ from: state.account!.username, text, at: Date.now() });
  ...
  const messages = bundles.map((bundle) => {
    ...
    if (stored) {
      ...
      const payload = encryptText(session, plaintext);
    ...
    const payload = encryptText(session, plaintext);
    conversation.sessions[bundle.identityKey] = serializeState(session);
    return { deviceId: bundle.deviceId, payload, invite };
  });
  ...
  conversation.messages.push({ from: state.account!.username, text, at: Date.now(), mine: true });
  await persistVault();
}
...
export async function receiveMessages(): Promise<number> {
  ...
  for (const envelope of envelopes) {
    try {
      ...
      const plaintext = JSON.parse(opened.plaintext) as { from: string; text: string; at: number };
